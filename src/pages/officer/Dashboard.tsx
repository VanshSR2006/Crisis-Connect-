...
export const Dashboard: React.FC = () => {
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      sub: `${dispatches.length} total dispatch orders`,
      icon: Radio,
    ...
    },
    {
      ...
      sub: highestRiskZone ? `Zone ${highestRiskZone.zone_id}` : 'Delhi NCR Basin',
      icon: Activity,
    ...
    },
    {
      ...
      sub: 'Multilingual emergency alerts live',
      icon: Users,
      valueColor: 'text-[#0f172a]',
  ...
  const quickNavLinks = [
    { label: 'Live GIS Map', path: '/officer/live-map', icon: MapIcon, desc: 'Real-time spatial incident tracking' },
    { label: 'Incident List', path: '/officer/incidents', icon: FileText, desc: 'Manage & filter active reports' },
    { label: 'Dispatch Center', path: '/officer/dispatch', icon: Radio, desc: 'Assign rescue & relief units' },
    { label: 'Risk Heatmap', path: '/officer/risk-heatmap', icon: Activity, desc: 'AI flood vulnerability scores' },
    { label: 'Analytics', path: '/officer/statistics', icon: BarChart3, desc: 'Throughput & response metrics' },
  ];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[13px] text-[#45464d] mt-0.5">
            National Emergency Operations Center · Delhi NCR
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white border border-[#c6c6cd] rounded overflow-hidden shadow-sm flex flex-col">
          <div className="px-3 py-2 border-b border-[#c6c6cd] flex items-center justify-between bg-[#f0edef]">
            ...
            >
              Open Interactive GIS Map <ChevronRight className="h-3.5 w-3.5" />
            </button>
          </div>
        ...
        <div className="bg-white border border-[#c6c6cd] rounded overflow-hidden shadow-sm flex flex-col">
          <div className="px-3 py-2 border-b border-[#c6c6cd] flex items-center justify-between bg-[#f0edef]">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[12px] font-semibold uppercase tracking-[0.05em] text-[#1b1b1d]">
                Critical Queue ({incidents.length})
              </span>
            </div>
            <button
              onClick={() => navigate('/officer/incidents')}
            ...
            {recentIncidents.map((incident, idx) => (
              ...
              >
                ...
                <div className="flex items-center justify-between mt-2 text-[10px] text-[#76777d]">
                  <span className="uppercase font-semibold tracking-wider text-[#0f172a]">{incident.status}</span>
