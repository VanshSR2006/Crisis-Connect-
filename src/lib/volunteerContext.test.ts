...
import { getDispatchesForVolunteer, mergeDispatchesWithIncidents } from '@/lib/volunteerContext';
...
describe('volunteer dispatch selection', () => {
  ...
  it('keeps a dispatch visible when its incident is missing without fabricating incident data', () => {
    expect(mergeDispatchesWithIncidents([dispatches[0]], [])).toEqual([
      { ...dispatches[0], incident: null },
    ]);
  });
});
