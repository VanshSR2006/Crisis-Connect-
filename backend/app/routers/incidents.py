...
async def create_incident(
    ...
    created_at_str = (
        new_inc.created_at.isoformat().replace("+00:00", "Z")
        if new_inc.created_at
...
async def verify_incident(
    ...
    """
    Verifies or flags an incident.

    Recalculates response priority dynamically and
    broadcasts an incident.verified event.
    """
    ...
    if req.review_state not in ["verified", "flagged"]:
        raise HTTPException(
            status_code=400,
            detail="Invalid review_state. Must be 'verified' or 'flagged'"
        )
