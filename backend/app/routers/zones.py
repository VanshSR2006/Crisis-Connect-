...
def get_zone_population(
    ...
    profile = db.query(PopulationProfile).filter(
        PopulationProfile.zone_id == id
    ).first()
...
def get_zone_demand(
    ...
    pop_profile = db.query(PopulationProfile).filter(
        PopulationProfile.zone_id == id
    ).first()
    ...
    households = (
        pop_profile.households_est
        if pop_profile
        else int(pop / 4)
    )

    vulnerability = (
        pop_profile.vulnerability_index
        if pop_profile
        else 0.65
    )

    # Calculate demographic demand
    demand = calculate_demographic_demand(
        pop,
        households,
        vulnerability
    )
    ...
    for resource_type, quantity_needed in forecast_data.items():
        ...
        if forecast:
            ...
            forecast.computed_at = datetime.utcnow()
